...
class TimeControl extends ValueControl {
	...
	public function Render(){
		...
		$c = SelectBox::Make($this->name.'_hours',$h)->WithOnChange($this->name.'_UpdateTime();')->WithAllowNull($this->allow_null)->WithWidth('4em')->WithCssStyle('text-align:center;')->WithMode($this->mode)->WithReadOnly($this->is_readonly);
		for ($i = 0; $i<24; $i++)
			$c->Add(($i<10?'0':'') . $i);
		$c->Render();

		echo ':';
		$c = SelectBox::Make($this->name.'_minutes',$m)->WithOnChange($this->name.'_UpdateTime();')->WithAllowNull($this->allow_null)->WithWidth('4em')->WithCssStyle('text-align:center;')->WithMode($this->mode)->WithReadOnly($this->is_readonly);
		for ($i = 0; $i<60; $i++)
			$c->Add(($i<10?'0':'') . $i);
		$c->Render();
		...
		if ($this->show_seconds){
			echo ':';
			$c = SelectBox::Make($this->name.'_seconds',$s)->WithOnChange($this->name.'_UpdateTime();')->WithAllowNull($this->allow_null)->WithWidth('4em')->WithCssStyle('text-align:center;')->WithMode($this->mode)->WithReadOnly($this->is_readonly);
			for ($i = 0; $i<60; $i++)
				$c->Add(($i<10?'0':'') . $i);
			$c->Render();
		}
