...
class TimeSpanBox extends Box {
	...
	public function Render(){
		...
		if ($this->show_days){
			echo TextBox::Make($this->name.'_days',$d)->WithWidth('4em')->WithCssClass('contentbox hcenter')->WithOnChange($this->name.".Update();")->WithMode($this->mode)->WithReadOnly($this->readonly);
			echo '&nbsp;'.Lemma::Pick('d.').'&nbsp;';
		}
		if ($this->show_hours){
			echo TextBox::Make($this->name.'_hours',$h)->WithWidth('3em')->WithCssClass('contentbox hcenter')->WithOnChange($this->name.".Update();")->WithMode($this->mode)->WithReadOnly($this->readonly);
			echo '&nbsp;'.Lemma::Pick('h.').'&nbsp;';
		}
		if ($this->show_minutes){
			echo TextBox::Make($this->name.'_minutes',$m)->WithWidth('3em')->WithCssClass('contentbox hcenter')->WithOnChange($this->name.".Update();")->WithMode($this->mode)->WithReadOnly($this->readonly);
			echo '&nbsp;&prime;';
		}
		if ($this->show_seconds){
			echo '&nbsp;';
			echo TextBox::Make($this->name.'_seconds',$s)->WithWidth('3em')->WithCssClass('contentbox hcenter')->WithOnChange($this->name.".Update();")->WithMode($this->mode)->WithReadOnly($this->readonly);
			echo '&nbsp;&Prime;';
		}
