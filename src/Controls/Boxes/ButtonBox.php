...
class ButtonBox extends Box {
  ...
  public function Render(){
	  ...
	  if ($this->is_rich) {
		  echo '<button type="'.$type.'" id="'.$this->name.'" name="'.$this->http_name.'" onclick="'.new Html($this->on_click).'"'.($this->is_disabled?' class="formButton disabled formButtonDisabled '.$this->css_class.'" disabled="disabled"':' class="formButton '.$this->css_class.'"').' style="'.$this->css_style.'">'.$this->value.'</button>';
	  }
	  else {
			echo '<input type="'.$type.'" id="'.$this->name.'" name="'.$this->http_name.'" value="'.$this->value.'" onclick="'.new Html($this->on_click).'"'.($this->is_disabled?' class="formButton disabled formButtonDisabled '.$this->css_class.'" disabled="disabled"':' class="formButton '.$this->css_class.'"').'  style="'.$this->css_style.'"/>';
	  }
  }
