...
class Profiler {
	...
	public static function Analyse($results){
		...
		$b[1] = array_sum(array_map( function($x){ return $x[1]; }, $a));
    ...
    echo '<table class="system" cellpadding="0" cellspacing="0" border="0">';
		...
		foreach ($a as $key=>$aa){
			...
			echo '<td class="hright">'.sprintf('%.3f',$aa[0]/$b[0]*100).'%</td>';
			echo '<td class="hright">'.$aa[1].'</td>';
			echo '<td class="hright">'.sprintf('%.3f',$aa[1]/$b[1]*100).'%</td>';
			echo '<td class="hright">'.$aa[2].'</td>';
			echo '<td class="hright">'.sprintf('%.3f',$aa[2]/$b[2]*100).'%</td>';
			echo '</tr>';
		}
