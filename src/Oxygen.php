...
class Oxygen {
	...
	public static function Init(){
		...
		$force_upgrade = array_key_exists('oxy_upgrade',$_GET);
		Database::Upgrade($force_upgrade,$logger);
		if ($force_upgrade || !$logger->IsEmpty()){
			if (!CLI) echo '<br/><pre>Upgrade complete. Please refresh.</pre>';
			die;
		}
				...
				if ($cli) {
					if ($ex instanceof ApplicationException)
						...
						for ($exx = $ex->getPrevious();!is_null($exx);$exx = $exx->getPrevious()) {
							echo $exx->getMessage() ,"\n";
							echo Debug::GetExceptionTraceAsText($exx) ,"\n";
						}
						//echo Oxygen::GetInfoAsText(),"\n\n";
						//echo Debug::GetEntriesAsText(),"\n\n";
						//echo Database::GetQueriesAsText(),"\n\n";
					}
				}
				else {
					$Q = "<!--\n\n\n\n\n\nEXCEPTION\n-->";
	...
	public static function AddLang($lang,$default_lang=null) { if (!in_array($lang,self::$langs,true)) { self::$langs[] = $lang; self::$default_langs[$lang] = ($default_lang===$lang?null:$default_lang); if (count(self::$langs)==1) self::$lang = $lang; } }
