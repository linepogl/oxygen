...
abstract class _oxy {
	...
	public static function AbbrDateTime( XDateTime $date_time = null , $default_value = ''){ return $date_time===null ? $default_value : '<abbr s="'.$date_time->Format('YmdHis').'" title="'. new XmlAttr(Lemma::Pick('xAgo')->Sprintf(static::FormatTimeSpanSince($date_time))."\n".Oxygen::GetClientTimeZone()).'">'.new Html(static::FormatDateTime($date_time)).'</abbr>'; }
	public static function AbbrDateTime24( XDateTime $date_time = null , $default_value = ''){ return $date_time===null ? $default_value : '<abbr s="'.$date_time->Format('YmdHis').'" title="'. new XmlAttr(Lemma::Pick('xAgo')->Sprintf(static::FormatTimeSpanSince($date_time))."\n".Oxygen::GetClientTimeZone()).'">'.new Html(static::FormatDateTime24($date_time)).'</abbr>';	}
	public static function AbbrDaysSince( XDateTime $date_time = null , $default_value = '' ){ if ($date_time===null) return $default_value; /** @var $diff XTimeSpan */ return '<span style="display:none;">'.sprintf('%020d',XDateTime::Now()->Diff( $date_time )->GetTotalMilliSeconds()).'</span><abbr title="'. new XmlAttr(static::FormatDateTime($date_time).' - '.Lemma::Pick('xAgo')->Sprintf(static::FormatTimeSpanSince($date_time))).'">'.static::FormatDaysSince($date_time).'</abbr>'; }
	public static function AbbrDate( XDateTime $date_time = null ){ return $date_time===null ? null : '<abbr s="'.$date_time->Format('YmdHis').'" title="'.new XmlAttr($date_time instanceof XDate ? static::FormatDate($date_time) . "\n" . Lemma::Pick('xAgo')->Sprintf( static::FormatDaysSince($date_time) ) : static::FormatDateTime($date_time) . "\n" . Lemma::Pick('xAgo')->Sprintf( static::FormatTimeSpanSince($date_time) ."\n".Oxygen::GetClientTimeZone() )).'">'.new Html(static::FormatDate($date_time)).'</abbr>'; }
	public static function AbbrByteRate( $byterate ){ return '<span style="display:none;">'.sprintf('%030d',$byterate).'</span><abbr title="'. new XmlAttr(Language::FormatNumber($byterate).' '.Lemma::Pick('unit:byte').'/'.Lemma::Pick('unit:sec')).'" class="nowrap">'.Language::FormatBytes($byterate).'/'.Lemma::Pick('unit:sec').'</abbr>'; }
	public static function AbbrByte( $bytes ){ return '<span style="display:none;">'.sprintf('%030d',$bytes).'</span><abbr title="'. new XmlAttr(Language::FormatNumber($bytes).' '.Lemma::Pick('unit:byte')).'" class="nowrap">'.Language::FormatBytes($bytes).'</abbr>'; }
