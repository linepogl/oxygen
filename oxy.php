...
abstract class _oxy extends _oxy_dictionary {
	...
	public static function AbbrDaysSince( XDateTime $date_time = null , $default_value = '' ){ if ($date_time===null) return $default_value; /** @var $diff XTimeSpan */ return '<span style="display:none;">'.sprintf('%020d',XDateTime::Now()->Diff( $date_time )->GetTotalMilliSeconds()).'</span><abbr title="'. new XmlAttr(static::FormatDateTime($date_time).' - '.static::FormatTimeSpanSince($date_time)).'">'.static::FormatDaysSince($date_time).'</abbr>'; }
